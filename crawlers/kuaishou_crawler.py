...
class KuaishouCrawler(BaseCrawler):
    ...
    async def search(self, keyword: str, max_pages: int = 5) -> List[dict]:
        ...
        try:
            ...
            for page_num in range(max_pages):
                logger.info(f"快手搜索 第{page_num + 1}页")

                # 获取视频列表
                video_items = await self.page.query_selector_all('[class*="video-card"], [class*="FeedItem"]')

                for item in video_items:
                    try:
                        # 提取标题
                        title_el = await item.query_selector('[class*="title"], [class*="caption"], a')
                        ...
                        # 提取作者
                        author_el = await item.query_selector('[class*="author"], [class*="nickname"], [class*="name"]')
    ...
    async def get_comments(self, url: str, max_count: int = 100) -> List[CommentData]:
        ...
        try:
            ...
            title = ""
            try:
                title_el = await self.page.query_selector('[class*="title"], [class*="caption"], h1')
                title = await title_el.inner_text() if title_el else ""
            except:
                pass
